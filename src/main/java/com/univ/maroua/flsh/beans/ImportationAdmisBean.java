 ...
 * @author lappa
 */
@ManagedBean(name = "importationAdmisBean")
@SessionScoped
@ViewScoped
public class ImportationAdmisBean implements Serializable {
    ...
    List<Reussite> generic() throws ServiceException {
        ...
        reportName = "Admis" + sec.findById(sectionId).getSigle() + (ss.getNom().equals("/") ? "" : ss.getNom()) + niv.findById(niveauId).getLevel();
        ...
        for (SpecialiteEtudiant specialiteEtudiant : specialites) {
            try {
                ...
                if (specialiteEtudiant.getChevauche() == 1) { //on a calcule les chevauchements apres
                    continue;
                }
            ...
            reussite.setAnnee("(Année académique " + anAc.next(anAc.findById(anneeAcademiqueid).getAnnee()).getAnnee() + ")");
            ...
            reussite.setMessage("Sont declarés admis au niveau " + nextLevel + " et  pour le compte de l'année académique " + anAc.next(anAc.findById(anneeAcademiqueid).getAnnee()).getAnnee() + " les candidats dont les noms suivent et classés par ordre alphabétique.");
        ...
        logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "edition de la liste des étudiants admis de la spécialité " + ss + " de l'année " + aa);
        return reussites;
    }
    ...
    public List<Etudiant> getEtudiants() {
        ...
        try {
            ...
            Logger.getLogger(ImportationAdmisBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return etudiants;
    }
