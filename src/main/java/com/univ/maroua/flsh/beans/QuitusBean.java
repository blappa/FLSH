 ...
 * @author lappa
 */
@ManagedBean(name = "quitusBean")
@SessionScoped
public class QuitusBean implements Serializable {
    ...
    public void init(String nom) throws JRException, ServiceException {
        ...
        jasperPrint = JasperFillManager.fillReport(path + File.separator + "recu" + File.separator + nom + ".jasper", new HashMap(), beanCollectionDataSource);
    }

    public void PDF(ActionEvent actionEvent) throws JRException, IOException, ServiceException {
        init("recu");

        logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "edition des quitus");
    ...
    List<Attestation> generic(int token) throws ServiceException {
        ...
        Float mgp = 0.0F;
        int credits = 0;
        AnneeAcademique anneeAcad = anAc.findById(anneeAcademiqueid);
        etudiants = new LinkedList<Etudiant>();
        attestations = new ArrayList<Attestation>();
        Specialite ss = spe.findById(specialiteId);
        reportName = "recu" + sec.findById(sectionId).getSigle() + (ss.getNom().equals("/") ? "" : ss.getNom()) + niv.findById(niveauId).getLevel();
        ...
        if (token == 0) {
            try {
                ...
                reportName = "recu" + etudiant.getNom();
                reportName = reportName.replaceAll(" ", "");
            ...
            if (typeOperation.equals("1")) {
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "impression du quitus de la premiere tranche de l'etudiant  : " + etudiant + " pour l'annee " + aa);
            } else if (typeOperation.equals("2")) {
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "impression du quitus de la seconde tranche de l'etudiant  : " + etudiant + " pour l'annee " + aa);
            } else {
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "impression du quitus de totalite de l'etudiant  : " + etudiant + " pour l'annee " + aa);
            }


        } else {
            Specialite spps = spe.findById(specialiteId);
            AnneeAcademique aa = anAc.findById(anneeAcademiqueid);
            if (typeOperation.equals("1")) {
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "impression du quitus de premiere tranche de la specialite  : " + spps + " pour l'annee " + aa);
            } else if (typeOperation.equals("2")) {
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "impression du quitus de seconde tranche de la specialite  : " + spps + " pour l'annee " + aa);
            } else {
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "impression du quitus de totalite de la specialite  : " + spps + " pour l'annee " + aa);
            }
        ...
        quitus = anneeAcad.getQuitus();
        String code;
        for (Etudiant etudiant1 : etudiants) {
            SpecialiteEtudiant spe = spetud.findByEtudiantSpecialiteAnnee(etudiant1.getId(), anneeAcademiqueid, specialiteId);
            if (spe.getCodeQ() == null) {
                quitus++;
                String star = anneeAcad.getAnnee().substring(2, 4) + "/";
                String end = "/DU";
                String middle = quitus + "";
                ...
                code = "FLSH/" + star + middle + end;
                spe.setCodeQ(code);
                SpecialiteEtudiant se = (SpecialiteEtudiant) spe.clone();
                spetud.delete(spe);
                se.setId(null);
                spetud.create(se);
            ...
            try {
                attestation.setNationalite(formatNat(etudiant1.getNationalite()));
            } catch (NullPointerException ex) {
            }
            attestation.setAnnee(annee);
            if (level < 4) {
                attestation.setNiveau("Licence " + fomatRomain(level));
            } else {
                attestation.setNiveau("Master " + fomatRomain(level));
            }
            attestation.setNomPrenom(etudiant1.getNom());
            attestation.setSexe(etudiant1.getSexe());

            attestation.setNo(code);
            if (etu.esNouveau(etudiant1.getId(), specialiteId, anneeAcademiqueid)) {
                attestation.setRedoublant("Non");
            } else {
                attestation.setRedoublant("Oui");
            }
            if (typeOperation.equals("1")) {
                switch (specialite.getProfessional()) {
                    case 0:
                        attestation.setTranche("Tranche 1: 25 000 FCFA");
                        break;
                    case 1:
                        attestation.setTranche("Tranche 1: 150 000 FCFA");
                        break;
                    case 2:
                        attestation.setTranche("Tranche 1: 250 000 FCFA");
                        break;
                }
                ...
                switch (specialite.getProfessional()) {
                    case 0:
                        attestation.setTranche("Tranche 2: 25 000 FCFA");
                        break;
                    case 1:
                        attestation.setTranche("Tranche 2: 150 000 FCFA");
                        break;
                    case 2:
                        attestation.setTranche("Tranche 2: 250 000 FCFA");
                        break;
                }
            } else if (typeOperation.equals("3")) {
                switch (specialite.getProfessional()) {
                    case 0:
                        attestation.setTranche("Totalité: 50 000 FCFA");
                        break;
                    case 1:
                        attestation.setTranche("Totalité: 300 000 FCFA");
                        break;
                    case 2:
                        attestation.setTranche("Totalité: 500 000 FCFA");
                        break;
                }
            }
            ...
            try {
                ...
                attestation.setDateNais(etudiant1.getDateNais() + "  à  " + lieu);

            } catch (IndexOutOfBoundsException ex) {
            } catch (NullPointerException e) {
            }
            attestation.setSpecialite(specialite.getNiveau().getSection().getNom());
            attestations.add(attestation);
            i++;
        }
        Collections.sort(attestations);
        anneeAcad.setQuitus(quitus);
        anAc.update(anneeAcad);
        JsfUtil.addSuccessMessage("en tout " + i + " certificats trouvees");
        return attestations;
    }
    ...
    public List<Etudiant> getEtudiants() {
        ...
        try {
            ...
            Logger.getLogger(QuitusBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return etudiants;
    }
    ...
    public String fomatRomain(Long level) {
        if (level == 1F || level == 4F) {
            return "I";
        }
        if (level == 2F || level == 5F) {
            return "II";
        }
        if (level == 3F) {
            return "III";
