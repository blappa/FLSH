...
@SessionScoped
@ViewScoped
public class CertificatBean implements Serializable {
    ...
    public void init(String nom) throws JRException, ServiceException {
        ...
        jasperPrint = JasperFillManager.fillReport(path + File.separator + "certificat" + File.separator + nom + ".jasper", new HashMap(), beanCollectionDataSource);
    }
    ...
    List<Attestation> generic(int token) throws ServiceException {
        ...
        if (token == 0) {
            try {
                ...
                SpecialiteEtudiant sp = new SpecialiteEtudiant();
                List<SpecialiteEtudiant> spss = spetud.findByEtudiantSectionNiveauAnnee(matricule, niveauLevel, sectionId, anneeAcademiqueid);
                if (spss.size() != 1) {
                ...
                etudiant = sp.getEtudiant();
                etudiants.add(etudiant);
                reportName = "certificat" + niveauLevel + etudiant.getNom();
                reportName = reportName.replaceAll(" ", "");
                AnneeAcademique aa = anAc.findById(anneeAcademiqueid);
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "edition du certificat de scolarité de l'étudiant " + etudiant + " pour l'année " + aa);
            ...
            logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "édition des certificats de scolarité de la specialite " + ss + "section " + sectionId + "niveau " + level);
            reportName = "certificat" + sec.findById(sectionId).getSigle() + (ss.getNom().equals("/") ? "" : ss.getNom()) + niv.findById(niveauId).getLevel();
            reportName = reportName.replaceAll(" ", "");
            level = Long.valueOf(niv.findById(niveauId).getLevel());
            etudiants = etu.findBySpecialiteAnnee(specialiteId, anneeAcademiqueid);
            AnneeAcademique aa = anAc.findById(anneeAcademiqueid);
            if (!etudiants.isEmpty()) {
                logger.info("#utilisateur=" + UserUtil.getUsername() + "#action=" + "édition des certificats de scolarité des étudiants de la specialité " + ss + " de l'année " + aa);
            }
        }
        int i = 0;
        ...
        Long idSect = null;
        if (!annee.equals("2014/2015")) {
            idSect = sectionId;
        }

        Long sem2 = getSemestres().get(1).getId();
        ...
        for (Etudiant etudiant1 : etudiants) {
            ...
            try {
                ...
                attestation.setLieuNais("    à  " + lieu);
            } catch (IndexOutOfBoundsException ex) {
            } catch (NullPointerException e) {
            }
    ...
    public List<Etudiant> getEtudiants() {
        ...
        try {
            ...
            Logger.getLogger(CertificatBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return etudiants;
    }
